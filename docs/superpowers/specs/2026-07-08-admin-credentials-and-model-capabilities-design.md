...
实现更新（2026-07-09）：本设计完成后，生产化加固又补齐了账号冷却调度、结果水合、参数白名单、统一错误、API Key 限流/配额、请求幂等和审计日志。下面只保留仍然成立的后续项：

- 账号池维护：结合余额、每日奖励、模型能力、失败率和冷却时间做自动补号、补签和禁用策略；当前只完成基础调度与冷却。
- 任务生命周期：在已支持 SSE/历史消息结果水合的基础上，补异步任务状态刷新、失败重试和取消。
- 高级视频场景验证：`reference`、`frame_based`、`motion` 已有协议构造和本地校验，但仍需要真实账号低成本成功样本。
